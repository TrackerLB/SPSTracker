...
class ConvProblem(optimization.L2Problem):
    def __init__(self, training_samples: TensorList, y: TensorList, filter_reg: torch.Tensor, sample_weights: TensorList, response_activation,size):
        self.training_samples = training_samples
        self.y = y
        self.filter_reg = filter_reg
        self.sample_weights = sample_weights
        ...
        self.device = 'cuda'
        self.output_window = dcf.hann2d(self.output_sz.long(), centered=False).to(self.device)
    def __call__(self, x: TensorList):
        """
        Compute residuals
        :param x: [filters]
        ...
        scores = scores[0] + self.pool6(scores_[0]).repeat(1, 1, self.size[0], 1) + self.pool7(scores_[0]).repeat(1, 1, 1, self.size[0])
