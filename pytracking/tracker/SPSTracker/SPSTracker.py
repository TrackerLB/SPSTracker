...
class SPSTracker(BaseTracker):

    def initialize_features(self,layer):
        if not getattr(self, 'features_initialized', False):
            self.params.features.initialize(layer)
    ...
    def initialize(self, image, state, gt,*args, **kwargs):
        ...
        if state[3]>50 or state[2]>50:
            ...
            self.target_sz = torch.Tensor([state[3]-state[3]/8, state[2]-state[2]/4])
        else:
            self.target_sz = torch.Tensor([state[3], state[2]])
        # Set search area
    ...
    def init_optimization(self, train_x, init_y):
        ...
        # Initialize optimizer
        self.conv_problem = ConvProblem(self.training_samples, self.y, self.filter_reg, self.sample_weights, self.response_activation,self.feature_sz[0])
    ...
    def localize_advanced(self, scores):
        ...
        scores = scores + self.pool1(scores).repeat(1, 1, 224, 1) + self.pool2(scores).repeat(1, 1, 1, 224)
    ...
    def refine_target_box(self, sample_pos, sample_scale, scale_ind, update_scale = True):
        ...
        self.target_sz = 0.3*new_target_sz + 0.7*self.target_sz  
        if update_scale:
