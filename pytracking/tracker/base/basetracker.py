...
from evaluation.utils.pyvotkit.region import vot_overlap
import torch 
from util.load_helper import load_pretrain
import torch
class BaseTracker:
    ...
    def track_sequence(self, sequence):
        ...
        for f, frame in enumerate(sequence.frames):
            #image = self._read_image(frame)
            if f==start_frame:
                image = self._read_image(frame)
                
                self.initialize(image, sequence.ground_truth_rect[f,:],sequence.gt[f,:])
                #print(siammask)
                ...
                #print(self.img_support_sz , self.output_sz)
                #print(image.shape)
                gt = sequence.gt
                if len(gt[f]) == 8:
                    ...
                    gt_polygon = ((gt[f][0], gt[f][1]),
                            (gt[f][0] + gt[f][2], gt[f][1]),
                            (gt[f][0] + gt[f][2], gt[f][1] + gt[f][3]),
                            (gt[f][0], gt[f][1] + gt[f][3]))
                #print(f,gt_polygon)
                pred_polygon = ((state[0], state[1]),
                            (state[0] + state[2], state[1]),
                            (state[0] + state[2], state[1] + state[3]),
                            (state[0], state[1] + state[3]))
                # pred_polygon = ((state[0], state[1]), (state[2], state[3]),
